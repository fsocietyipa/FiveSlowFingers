...
class GameVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == wordIndex {
            cell.backgroundColor =  #colorLiteral(red: 0.7960784314, green: 0.7960784314, blue: 0.7960784314, alpha: 1)
        }
        else {
        ...
        if previuosWordsIndex.contains(indexPath.row) && previousCorrect[indexPath.row] == true {
            cell.textLabel?.textColor = #colorLiteral(red: 0.03529411765, green: 0.4901960784, blue: 0.09803921569, alpha: 1)
        }
        else if previuosWordsIndex.contains(indexPath.row) && previousCorrect[indexPath.row] == false {
            cell.textLabel?.textColor = #colorLiteral(red: 0.9647058824, green: 0.05882352941, blue: 0.0862745098, alpha: 1)
        }
        else {
        ...
        //cell.textLabel?.font = UIFont(name:"Bebas", size: 20.0)
        cell.textLabel?.text = selectedWords[indexPath.row]
        return cell
    }
